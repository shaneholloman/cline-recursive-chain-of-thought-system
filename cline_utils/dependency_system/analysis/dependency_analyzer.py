...
@cached(
    "file_analysis",
    key_func=lambda file_path,
    force=False: f"analyze_file:{normalize_path(file_path)}:{(os.path.getmtime(file_path) if os.path.exists(file_path) else 0)}:{force}",
)
def analyze_file(file_path: str, force: bool = False) -> Dict[str, Any]:
    ...
    if (
        any(is_subpath(norm_file_path, excluded) for excluded in excluded_dirs_abs.union(excluded_paths_abs))
        or os.path.splitext(norm_file_path)[1].lower().lstrip(".") in excluded_extensions
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if target_full_name:
                    result["calls"].append(
                        {"target_name": target_full_name, "potential_source": potential_source, "line": node.lineno}
                    )
                ...
                if potential_source:
                    result["attribute_accesses"].append(
                        {"target_name": attribute_name, "potential_source": potential_source, "line": node.lineno}
                    )
...
def _analyze_javascript_file(file_path: str, content: str, result: Dict[str, Any]) -> None:
    ...
    try:  # Function/Class regex
        ...
        logger.warning(f"Regex error during JS analysis in {file_path}: {e}")
