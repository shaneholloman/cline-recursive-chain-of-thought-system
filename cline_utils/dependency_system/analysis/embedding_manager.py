    ...
    if SELECTED_DEVICE is None:
        ...
        if config_device in ["cuda", "mps", "cpu"]:
            ...
            if config_device == "cuda" and not torch.cuda.is_available():
                ...
                logger.warning(
                    f"Config specified 'mps', but MPS is not available/built. Falling back to auto-detection."
                )
                SELECTED_DEVICE = _get_best_device()
            else:
            ...
            logger.info("Auto-detecting device.")
            SELECTED_DEVICE = _get_best_device()
        else:
...
def _load_model():
    ...
    if MODEL_INSTANCE is None:
        ...
        try:
            ...
            logger.error(
                f"Failed to import SentenceTransformer: {e}. Please install it (`pip install sentence-transformers`)"
            )
            raise
    ...
    if file_path.lower().endswith(".py"):
        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                if segment:
                    weighted_definitions.extend([segment, segment])  # Weighting
        except Exception as e:
            logger.warning(f"AST weighting failed for {file_path}: {e}. Proceeding without.")
...
def generate_embeddings(
    ...
    if not project_paths:
        logger.error("No project paths provided.")
        return False
    if not path_to_key_info:
        logger.warning("path_to_key_info map is empty. Cannot generate embeddings.")
        return False
    ...
    for relative_path in project_paths:
        ...
        if not is_valid_project_path(current_project_path):
            logger.error(f"Invalid path skipped: {current_project_path}")
            overall_success = False
            continue

        # Determine metadata path (specific to this root path being processed)
        project_name = os.path.basename(current_project_path)
        project_embeddings_dir = normalize_path(
            os.path.join(embeddings_dir, project_name)
        )  # This subdir approach might be redundant now? Check if needed.
        metadata_file = normalize_path(
            os.path.join(project_embeddings_dir, "metadata.json")
        )  # If using mirrored structure, maybe one metadata file at base embeddings_dir?
        ...
        if not force and os.path.exists(metadata_file):
            try:
                ...
                if existing_metadata.get("version") != "1.0":
                    logger.warning(f"Incompatible metadata version in {metadata_file}. Regenerating.")
                    existing_metadata = {}
            except Exception as e:
                logger.warning(f"Corrupted/missing metadata {metadata_file}: {e}. Regenerating.")
                existing_metadata = {}
        ...
        key_info_for_current_path: Dict[str, KeyInfo] = {
            norm_path: info
            for norm_path, info in path_to_key_info.items()
            if info.norm_path.startswith(current_project_path)
            and not info.is_directory  # Only process files within this path
        }
        ...
        for norm_path, key_info in key_info_for_current_path.items():
            ...
            try:
                ...
                with open(abs_file_path, "r", encoding="utf-8") as f:
                    file_contents[key_string] = f.read()
        ...
        for norm_path, key_info in key_info_for_current_path.items():
            ...
            if metadata_key_entry and not force:
                ...
                try:
                    ...
                    mirrored_npy_path = normalize_path(
                        os.path.join(embeddings_dir, relative_file_path_for_npy) + ".npy"
                    )
                except ValueError:
                    mirrored_npy_path = None
                ...
                if mirrored_npy_path and os.path.exists(mirrored_npy_path):
                    try:
                        ...
                        if metadata_mtime is not None and current_mtime == metadata_mtime:
                            ...
                            logger.debug(f"  Meta mtime missing. Regen {key_string}.")
                        else:
                            logger.debug(f"  MTime mismatch. Regen {key_string}.")
                    except FileNotFoundError:
                        logger.warning(f"Source file {abs_file_path} gone. Skip {key_string}.")
                        should_generate = False
                    except Exception as e:
                        logger.warning(f"Mtime check failed for {key_string}: {e}. Regen.")
                        should_generate = True
                else:
                    logger.debug(f"NPY not found {mirrored_npy_path}. Gen {key_string}.")
                    should_generate = True
            elif force:
                logger.info(f"Force flag set. Regen {key_string}.")
                should_generate = True
            else:
                logger.debug(f"Key {key_string} not in meta or meta missing. Gen.")
                should_generate = True
                ...
                if processed_content.strip():
                    try:
                        ...
                        try:
                            ...
                            logger.error(f"Failed save embedding {key_string} ({abs_file_path}) to {save_path}: {e}")
                            overall_success = False
                    except Exception as e:
                        logger.error(f"Failed generate embedding {key_string} ({abs_file_path}): {e}")
                        overall_success = False
                else:
                    logger.debug(f"Skipping empty file content for key {key_string} ({abs_file_path})")
        ...
        for norm_path, key_info in key_info_for_current_path.items():
            ...
            if os.path.exists(abs_v_path) and (
                key_string in current_embeddings or key_string in keys_skipped_mtime_this_pass
            ):
                try:
                    ...
                    logger.warning(f"File {abs_v_path} gone before meta save for {key_string}.")
                except Exception as e:
                    logger.error(f"Error getting mtime for {abs_v_path} ({key_string}): {e}.")

        if not valid_keys_in_metadata:
            logger.warning(f"No valid files processed for metadata in {current_project_path}. Skipping save.")
        else:
            ...
            try:
                ...
                logger.error(f"Failed write metadata {metadata_file}: {e}")
                overall_success = False
...
# <<< *** MODIFIED SIGNATURE AND LOGIC *** >>>
@cached("similarity_calculation", key_func=_get_similarity_cache_key)  # Use helper function for key generation
def calculate_similarity(
    key1_str: str,  # Renamed for clarity
    ...
    if not (validate_key(key1_str) and validate_key(key2_str)):
        logger.warning(f"Invalid key format for similarity: {key1_str}, {key2_str}")
        return 0.0
    ...
    if not key1_info or not key2_info:
        missing_keys = []
        if not key1_info:
            missing_keys.append(key1_str)
        if not key2_info:
            missing_keys.append(key2_str)
    ...
    def get_embedding_path(key_str: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error calc rel path for {norm_abs_file_path}: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error getting embedding path for key {key_str}: {e}")
            return None
    ...
    if not file1_path or not file2_path or not (os.path.exists(file1_path) and os.path.exists(file2_path)):
        missing = []
        if not file1_path or not (file1_path and os.path.exists(file1_path)):
            missing.append(file1_path or f"{key1_str}.npy (path error)")
        if not file2_path or not (file2_path and os.path.exists(file2_path)):
            missing.append(file2_path or f"{key2_str}.npy (path error)")
        relative_missing = [
            os.path.relpath(m, embeddings_dir) if m and os.path.isabs(m) and embeddings_dir in m else m for m in missing
        ]
    ...
    try:
        ...
        if norm1 == 0 or norm2 == 0:
            logger.debug(
                f"Zero vector for {key1_str if norm1 == 0 else ''}{' and ' if norm1 == 0 and norm2 == 0 else ''}{key2_str if norm2 == 0 else ''}. Sim=0."
            )
            return 0.0
        similarity = float(np.dot(emb1, emb2) / (norm1 * norm2))
        return max(0.0, min(1.0, similarity))
    except Exception as e:
        logger.exception(f"Failed similarity calc for {key1_str} & {key2_str}: {e}")
        return 0.0
...
def _is_valid_file(file_path: str) -> bool:
    ...
    exclude_dirs = [
        normalize_path(os.path.join(project_root, d)) for d in exclude_dirs_raw
    ...
    exclude_files = [
        normalize_path(os.path.join(project_root, f)) for f in exclude_files_raw
    ]  # Normalize exclusion files
    ...
    if file_name.startswith("."):
        return False
    if any(norm_file_path.startswith(ex_dir + os.sep) or norm_file_path == ex_dir for ex_dir in exclude_dirs):
        return False  # Ensure checking prefix correctly
    ext = os.path.splitext(norm_file_path)[1].lower()
    if ext in exclude_exts:
        return False
    try:
        return os.path.isfile(norm_file_path) and os.path.getsize(norm_file_path) < 10 * 1024 * 1024
    except OSError:
        return False
...
def command_handler(args):
    ...
    logger.error(
        "Direct CLI command for generate-embeddings needs adaptation for contextual keys. Use project analysis flow."
    )
