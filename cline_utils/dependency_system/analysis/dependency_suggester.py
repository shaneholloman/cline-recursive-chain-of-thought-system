...
def suggest_dependencies(
    ...
    if file_ext == ".py":
        ...
        return suggest_javascript_dependencies(
            norm_path, path_to_key_info, project_root, file_analysis_results, threshold
        )
        ...
        return suggest_documentation_dependencies(
            norm_path, path_to_key_info, project_root, file_analysis_results, threshold, embeddings_dir, metadata_path
        )
    else:
...
def _identify_structural_dependencies(
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        ...
                        imported_name = alias.asname or module_name
                    ...
                    possible_base_paths = _convert_python_import_to_paths(
                        full_module_name_for_path,
                        resolve_from_dir,
                        project_root,
                        is_from_import=True,
    ...
    for call in calls:
        ...
        if target_key_string and target_key_string != source_key_string:
            logger.debug(
                f"Suggesting {source_key_string} -> {target_key_string} (>) due to call: {call.get('target_name')}"
            )
    ...
    for attr in attributes:
        ...
        if target_key_string and target_key_string != source_key_string:
            logger.debug(
                f"Suggesting {source_key_string} -> {target_key_string} (>) due to attr access: {attr.get('potential_source')}.{attr.get('target_name')}"
            )
            suggestions.append((target_key_string, ">"))
    ...
    for inh in inheritance:
        ...
        if target_key_string and target_key_string != source_key_string:
            logger.debug(
                f"Suggesting {source_key_string} -> {target_key_string} (<) due to inheritance from: {inh.get('base_class_name')}"
            )
            suggestions.append((target_key_string, "<"))
...
def suggest_python_dependencies(
    ...
    if analysis is None:
        logger.warning(f"No analysis results for {norm_file_path}")
        return []
    if "error" in analysis or "skipped" in analysis:
        logger.info(f"Skipping sugg for {norm_file_path} due to analysis error/skip.")
        return []

    # 1. Explicit import dependency ('>')
    explicit_suggestions = []
    # Pass path_to_key_info down
    explicit_deps_paths = _identify_python_dependencies(
    ...
    for dep_path, dep_type in explicit_deps_paths:
        ...
        if dep_key_string:
            ...
            if source_key_string and dep_key_string != source_key_string:
                logger.debug(
                    f"Suggesting {source_key_string} -> {dep_key_string} ({dep_type}) due to explicit Python import."
                )
...
def suggest_documentation_dependencies(
    ...
    analysis = file_analysis_results.get(norm_file_path)
    if analysis is None:
        logger.warning(f"Analysis results None for {norm_file_path}. Skipping.")
        return []
    if "error" in analysis:
        logger.warning(f"Analysis error for {norm_file_path}: {analysis['error']}. Skipping.")
        return []
...
# <<< *** MODIFIED SIGNATURE *** >>>
def suggest_semantic_dependencies(
    file_path: str,
    path_to_key_info: Dict[str, KeyInfo],  # Changed
    project_root: str,
) -> List[Tuple[str, str]]:
    ...
    if not os.path.exists(embeddings_dir):
        logger.warning(
            f"Embeddings directory not found at {embeddings_dir}. Cannot perform semantic analysis. Please run embedding generation."
        )
        return []
    ...
    target_key_infos = [
        info for info in path_to_key_info.values() if not info.is_directory and info.key_string != source_key_string
    ]
    ...
    try:
        ...
        logger.error("Could not import calculate_similarity. Semantic suggestions disabled.")
        return []
    ...
    for target_key_info in target_key_infos:
        ...
        if confidence >= threshold_S_strong:
            assigned_char = "S"
            logger.debug(f"Suggesting {source_key_string} -> {target_key_string} ('S')...")
...
def _combine_suggestions_with_char_priority(suggestions: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    ...
    for key, char in suggestions:
        ...
        if new_priority > current_priority:
            ...
            logger.debug(
                f"Combining suggestions for target {key}: Choosing '{char}' (prio {new_priority}) over '{current_char}' (prio {current_priority})."
            )
            combined[key] = char
            ...
            if {char, current_char} == {"<", ">"}:
                ...
                if combined.get(key) != "x":  # Avoid redundant logs/updates
                    logger.debug(
                        f"Combining suggestions for target {key}: Conflict between '<' and '>'. Setting to 'x'."
                    )
                    combined[key] = "x"
            else:
                # Keep the existing character for other equal priority conflicts (arbitrary, but consistent)
                logger.debug(
                    f"Combining suggestions for target {key}: Equal priority conflict between '{char}' and '{current_char}'. Keeping existing '{current_char}'."
                )
...
def _convert_python_import_to_paths(
    import_name: str, source_dir: str, project_root: str, is_from_import: bool = False, relative_level: int = 0
) -> List[str]:
    """
    ...
    if relative_level > 0:
        relative_module_part = import_name
...
def _identify_javascript_dependencies(
    source_path: str,
    source_analysis: Dict[str, Any],
    ...
    for import_path_str in imports:
        if (
            not import_path_str
            or not (import_path_str.startswith(".") or import_path_str.startswith("/"))
            or import_path_str.startswith("http:")
            or import_path_str.startswith("https:")
        ):
            continue
        try:
            ...
            for target_path_abs in possible_targets:
                ...
                if norm_target_path in tracked_paths_set:
                    dependencies.append((norm_target_path, ">"))
                    found = True
    ...
    for link in links:
        ...
        try:
            ...
            for target_path in possible_targets:
                ...
                # Check if tracked
                if norm_target_path in tracked_paths_set:
                    dependencies.append((norm_target_path, "d"))
                    found = True
                    break
...
def _identify_html_dependencies(
    ...
    urls_to_check = []
    for link in links:
        urls_to_check.append((link.get("href"), "link"))
    for script in scripts:
        urls_to_check.append((script.get("src"), "script"))
    for style in stylesheets:
        urls_to_check.append((style.get("href"), "style"))
    for img in images:
        urls_to_check.append((img.get("src"), "image"))
...
def _identify_css_dependencies(
    ...
    for import_item in imports:
        ...
        if not url or url.startswith("#") or ("://" in url) or (url.startswith("//")) or url.startswith("data:"):
            continue
        try:
            url_cleaned = url.split("#")[0].split("?")[0]
            if not url_cleaned:
