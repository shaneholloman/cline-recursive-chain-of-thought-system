...
def analyze_project(force_analysis: bool = False, force_embeddings: bool = False) -> Dict[str, Any]:
    ...
    if any(
        norm_project_root == excluded_path or norm_project_root.startswith(excluded_path + os.sep)
    ...
    try:
        ...
        results["status"] = "error"
        results["message"] = f"Key generation failed: {kge}"
        logger.critical(results["message"])
        return results
    except Exception as e:
        results["status"] = "error"
        results["message"] = f"Key generation failed unexpectedly: {e}"
    ...
    for key_info in path_to_key_info.values():
        if not key_info.is_directory:
            if key_info.parent_path:
                ...
                logger.warning(
                    f"File '{key_info.norm_path}' (Key: {key_info.key_string}) has no parent path. Cannot map to a module."
                )
    ...
    for abs_root_dir in abs_all_roots:
        ...
        for root, dirs, files in os.walk(abs_root_dir, topdown=True):
            ...
            if norm_root in all_excluded_paths_abs or any(
                is_subpath(norm_root, excluded) for excluded in all_excluded_paths_abs
            ):
    ...
    analysis_results_list = process_items(files_to_analyze_abs, analyze_file, force=force_analysis)
    ...
    try:
        ...
        results["status"] = "error"
        results["message"] = f"Dependency suggestion failed critically: {e}"
        logger.exception(results["message"])
        return results
    ...
    for norm_module_path, module_key_info in potential_mini_tracker_dirs.items():
        ...
        if os.path.isdir(norm_module_path) and not _is_empty_dir(norm_module_path):
            ...
            mini_tracker_path = tracker_io.get_tracker_path(
                project_root, tracker_type="mini", module_path=norm_module_path
            )
            logger.info(
                f"Updating mini tracker for module '{norm_module_path}' (Key: {module_key_string}) at: {mini_tracker_path}"
            )
    ...
    if results["status"] == "success":
        ...
        results["message"] = results.get("message", "") + " Project analysis completed with warnings."
    return results
