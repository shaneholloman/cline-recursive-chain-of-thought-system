...
# --- Grid Creation ---
@cached("initial_grids", key_func=lambda keys: f"initial_grid:{':'.join(sort_key_strings_hierarchically(keys))}")
...
# <<< *** MODIFIED SORTING *** >>>
@cached(
    "grid_validation",  # Caching needs careful review if inputs change frequently or if side effects (logging) matter
    key_func=lambda grid,
    keys: f"validate_grid:{hash(str(sorted(grid.items())))}:{':'.join(sort_key_strings_hierarchically(keys))}",
)  # Use hierarchical sort for keys part
def validate_grid(grid: Dict[str, str], sorted_keys_list: List[str]) -> bool:
    ...
    if not isinstance(grid, dict):
        ...
        return False
    if not isinstance(sorted_keys_list, list):
        logger.error("Grid validation failed: 'sorted_keys_list' not a list.")
        return False
    ...
    if num_keys == 0 and not grid:
        return True  # Empty grid and keys is valid
    if num_keys == 0 and grid:
        logger.error("Grid validation failed: Grid not empty but keys list is.")
        return False
    ...
    if missing_rows:
        logger.error(f"Grid validation failed: Missing rows for keys: {sorted(list(missing_rows))}")
        return False
    if extra_rows:
        logger.error(f"Grid validation failed: Extra rows found for keys: {sorted(list(extra_rows))}")
        return False
    ...
    for idx, key in enumerate(sorted_keys_list):
        ...
        if compressed_row is None:
            logger.error(f"Grid validation failed: Row missing for key '{key}'.")
            return False  # Should have been caught

        try:
            decompressed = decompress(compressed_row)
        except Exception as e:
            logger.error(f"Grid validation failed: Error decompressing row '{key}': {e}")
            return False

        if len(decompressed) != num_keys:
            logger.error(
                f"Grid validation failed: Row '{key}' length incorrect (Exp:{num_keys}, Got:{len(decompressed)})."
            )
            return False
        ...
        if decompressed[idx] != DIAGONAL_CHAR:
            logger.error(
                f"Grid validation failed: Row for key '{key}' has incorrect diagonal character at index {idx} (Expected: '{DIAGONAL_CHAR}', Got: '{decompressed[idx]}'). Row: '{decompressed}'"
            )
            return False
...
def add_dependency_to_grid(
    grid: Dict[str, str], source_key: str, target_key: str, keys: List[str], dep_type: str = ">"
) -> Dict[str, str]:
    """
    ...
    if source_idx == target_idx:
        ...
        raise ValueError(
            f"Cannot directly modify diagonal element for key '{source_key}'. Self-dependency must be 'o'."
        )
...
def remove_dependency_from_grid(
    grid: Dict[str, str], source_key: str, target_key: str, keys: List[str]
) -> Dict[str, str]:
    """
    ...
    invalidate_dependent_entries(
        "grid_validation", f"validate_grid:{hash(str(sorted(new_grid.items())))}:{':'.join(keys)}"
    )
    return new_grid
...
@cached(
    ...
    key_func=lambda grid,
    key,
    keys: f"grid_deps:{hash(str(sorted(grid.items())))}:{key}:{':'.join(sort_key_strings_hierarchically(keys))}",
)  # Use hierarchical sort for keys part
    ...
    for i, other_key in enumerate(keys):
        ...
        if row_key_compressed:
            try:
                ...
                pass  # Ignore if index out of bounds for this row
