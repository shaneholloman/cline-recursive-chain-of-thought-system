...
class BatchProcessor:
    ...
    def process_with_collector(
        self, items: List[T], processor_func: Callable[..., R], collector_func: Callable[[List[R]], Any], **kwargs: Any
    ) -> Any:
        """
    ...
    def _determine_batch_size(self) -> int:
        ...
        logger.debug(
            f"Adaptive batch size: Total={self.total_items}, Workers={effective_workers}, TargetBatches/Worker={target_batches_per_worker} => Calculated={calculated_batch_size}, Min={min_batch}, Max={max_batch} -> Final={final_batch_size}, Batches={num_batches}"
        )
        ...
        with ThreadPoolExecutor(max_workers=min(self.max_workers, len(batch))) as executor:
            ...
            for future in as_completed(future_to_idx):
                ...
                try:
                    ...
                    logger.error(
                        f"Error processing item (batch index {idx_in_batch}): {item_repr} -> {e}", exc_info=True
                    )  # Log with traceback
