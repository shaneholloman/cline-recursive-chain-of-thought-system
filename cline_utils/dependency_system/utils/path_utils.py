...
def get_relative_path(path: str, base_path: str) -> str:
    ...
    try:
        return os.path.relpath(norm_path, norm_base).replace("\\", "/")  # Ensure forward slashes
    except ValueError:
...
def is_valid_project_path(path: str) -> bool:
    ...
    @cached(
        "valid_project_paths", key_func=lambda p: f"valid_project_path:{normalize_path(p)}:{get_project_root()}"
