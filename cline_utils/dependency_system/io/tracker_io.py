...
@cached(
    "tracker_paths",
    key_func=lambda project_root,
    tracker_type="main",
    module_path=None: f"tracker_path:{normalize_path(project_root)}:{tracker_type}:{normalize_path(module_path) if module_path else 'none'}:{(os.path.getmtime(ConfigManager().config_path) if os.path.exists(ConfigManager().config_path) else 0)}",
)
...
@cached(
    "tracker_data",
    key_func=lambda tracker_path: f"tracker_data:{normalize_path(tracker_path)}:{(os.path.getmtime(tracker_path) if os.path.exists(tracker_path) else 0)}",
)
...
def write_tracker_file(
    ...
    try:
        ...
        for row_key in sorted_keys_list:
            compressed_row = grid_to_write.get(row_key)
            row_list = None
            if compressed_row is not None:
                try:
                    decompressed_row = decompress(compressed_row)
                    if len(decompressed_row) == expected_len:
                        row_list = list(decompressed_row)
                    else:
                        logger.warning(
                            f"Correcting grid row length for key '{row_key}' in {tracker_path} (expected {expected_len}, got {len(decompressed_row)})."
                        )
                except Exception as decomp_err:
                    logger.warning(
                        f"Error decompressing row for key '{row_key}' in {tracker_path}: {decomp_err}. Re-initializing."
                    )
            if row_list is None:
                ...
                if filename.startswith(base_name + ".") and filename.endswith(".bak"):
                    ...
                    if match:
                        ...
                        try:
                            ...
                            logger.warning(f"Could not parse timestamp for backup: {filename}")
...
def _merge_grids(
    primary_grid: Dict[str, str],
    secondary_grid: Dict[str, str],
    primary_keys_list: List[str],
    secondary_keys_list: List[str],
    merged_keys_list: List[str],
) -> Dict[str, str]:
        ...
        for key, compressed in grid_data.items():
            ...
            try:
                ...
                if len(decomp) == expected_len:
                    ...
                    logger.warning(
                        f"Merge Prep: Incorrect length for key '{key}' (expected {expected_len}, got {len(decomp)}). Skipping row."
                    )
            except Exception as e:
...
def merge_trackers(
    primary_tracker_path: str, secondary_tracker_path: str, output_path: Optional[str] = None
    ...
    logger.info(
        f"Attempting to merge '{os.path.basename(primary_tracker_path)}' and '{os.path.basename(secondary_tracker_path)}' into '{os.path.basename(output_path)}'"
    )
    ...
    if not primary_keys and not secondary_keys:
        ...
        logger.info(
            f"Primary tracker {os.path.basename(primary_tracker_path)} empty/unreadable. Using secondary tracker."
        )
        merged_data = secondary_data
    elif not secondary_keys:
        logger.info(
            f"Secondary tracker {os.path.basename(secondary_tracker_path)} empty/unreadable. Using primary tracker."
        )
        ...
        merged_compressed_grid = _merge_grids(
            primary_data.get("grid", {}),
            secondary_data.get("grid", {}),
    ...
    if write_tracker_file(
        ...
        merged_data["keys"],
        merged_data["grid"],
        merged_data["last_key_edit"],
        merged_data["last_grid_edit"],
    ):
...
def _write_key_definitions(file_obj: io.TextIOBase, key_map: Dict[str, str], sorted_keys_list: List[str]):
    ...
    for k in sorted_keys_list:  # Iterate pre-sorted list
        ...
        if v != "PATH_ERROR":
            ...
            logger.error(f"Path error writing key def: {k}")
            ...
            if compressed_row is not None:
                try:
                    ...
                    if len(decompressed) == expected_len:
                        final_compressed_row = compressed_row
                    else:
                        logger.warning(f"Correcting grid row length for key '{row_key}' before write...")
                except Exception:
                    logger.warning(f"Error decompressing row for key '{row_key}' before write...")
...
def create_mini_tracker(
    ...
    for k_str in relevant_keys_for_grid:
        ...
        if found_info:
            ...
            logger.warning(
                f"Key string '{k_str}' needed for mini-tracker '{os.path.basename(output_file)}' definitions not found in global path_to_key_info."
            )
    ...
    try:
        ...
        with open(output_file, "w", encoding="utf-8", newline="\n") as f:
            ...
            last_key_edit_msg = (
                f"Assigned keys: {', '.join(new_key_strings_for_this_tracker)}"
                if new_key_strings_for_this_tracker
                else (f"Initial key: {module_key_string}" if module_key_string else "Initial creation")
            )
...
def update_tracker(
    output_file_suggestion: str,  # Path suggestion (may be ignored for mini/main)
    ...
    if min_positive_priority <= 1:
        logger.warning("Min priority ('s') <= 1. Using 2.")
        min_positive_priority = 2
    ...
    if tracker_type == "main":
        ...
        if not force_apply_suggestions:
            ...
            try:
                ...
                for src_path, targets in aggregated_result_paths.items():
                    ...
                    src_key_string = src_key_info.key_string if src_key_info else None
                    if not src_key_string or src_key_string not in final_key_defs:
                        continue
                    for target_path, dep_char in targets:
                        target_key_info = path_to_key_info.get(target_path)
                        target_key_string = target_key_info.key_string if target_key_info else None
                ...
                logger.info(
                    f"Main tracker aggregation complete. Found {sum(len(v) for v in final_suggestions_to_apply.values())} relevant aggregated dependencies."
                )
            ...
            if suggestions:
                ...
                for src_key, deps in suggestions.items():
                    if src_key in final_key_defs:
                        ...
                        logger.warning(
                            f"Source key '{src_key}' from forced suggestion not found in main tracker keys. Skipping."
                        )
            else:
        ...
        logger.info(
            f"Doc tracker update. Definitions: {len(final_key_defs)} items. Grid keys: {len(relevant_keys_for_grid)}."
        )
        ...
        if not file_to_module:
            logger.error("file_to_module mapping required for mini-tracker updates.")
            return
        if not path_to_key_info:
            logger.warning("Global path_to_key_info is empty.")
            return
        ...
        if not module_key_info or not module_key_info.is_directory:
            ...
            if not module_key_info or not module_key_info.is_directory:
                logger.error(
                    f"Cannot determine valid module path/key from suggestion path: {output_file_suggestion} -> {potential_module_path}"
                )
                return
        ...
        existing_key_defs_mini = {}
        existing_grid_mini = {}
        tracker_exists_mini = os.path.exists(output_file)
        if tracker_exists_mini:
            try:
                with open(output_file, "r", encoding="utf-8") as f_mini:
                    mini_lines = f_mini.readlines()
                existing_key_defs_mini = _read_existing_keys(mini_lines)
                existing_grid_mini = _read_existing_grid(mini_lines)
            except Exception as e:
                logger.error(f"Mini Read Error: {e}")
                tracker_exists_mini = False  # Treat as non-existent if read fails

        # --- Determine Relevant Keys (Revised Logic) ---
        # 1. Identify Internal Keys
        internal_keys_info: Dict[str, KeyInfo] = {
            p: info for p, info in path_to_key_info.items() if info.parent_path == module_path or p == module_path
        }
        ...
        logger.debug(
            f"Mini Tracker ({module_key_string}): Initial relevant keys (internal): {len(relevant_keys_strings_set)}"
        )
        ...
        if tracker_exists_mini and existing_grid_mini and existing_key_defs_mini:
            ...
            for row_key, compressed_row in existing_grid_mini.items():
                if row_key not in old_key_to_idx_mini:
                    continue
                try:
                    ...
                    for col_idx, dep_char in enumerate(decomp_row):
                        if col_idx >= len(old_keys_list_mini):
                            break  # Safety break
                        ...
                        if dep_priority >= min_positive_priority:
                            if row_is_internal and not col_is_internal:
                                if col_key not in relevant_keys_strings_set:
                                    logger.debug(
                                        f"  Adding persisted foreign key '{col_key}' due to link from internal '{row_key}' ('{dep_char}')"
                                    )
                                ...
                                if row_key not in relevant_keys_strings_set:
                                    logger.debug(
                                        f"  Adding persisted foreign key '{row_key}' due to link to internal '{col_key}' ('{dep_char}')"
                                    )
            ...
            logger.debug(
                f"Mini Tracker ({module_key_string}): Relevant keys after scanning existing grid: {len(relevant_keys_strings_set)}"
            )
        ...
        excluded_dirs_abs = {
            normalize_path(os.path.join(project_root_for_exclude, p)) for p in config.get_excluded_dirs()
        }
        ...
        if raw_suggestions:
                ...
                if source_is_internal:
                    ...
                    for target_key_str, dep_char in deps:
                        if get_priority(dep_char) >= min_positive_priority:  # Only consider meaningful suggestions
                            target_info = next(
                                (info for info in path_to_key_info.values() if info.key_string == target_key_str), None
                            )
                            if target_info and target_info.norm_path not in all_excluded_abs:
                                if target_key_str not in relevant_keys_strings_set:
                                    logger.debug(
                                        f"  Adding suggested foreign key '{target_key_str}' linked from internal '{src_key_str}'"
                                    )
                                    relevant_keys_strings_set.add(target_key_str)

            all_target_keys_in_suggestions = {
                tgt
                for deps in raw_suggestions.values()
                for tgt, dep in deps
                if get_priority(dep) >= min_positive_priority
            }
            for target_key_str in all_target_keys_in_suggestions:
                if target_key_str in internal_keys_set:
                    ...
                    if target_path and target_path in all_excluded_abs:
                        continue
                    for src_key_str, deps in raw_suggestions.items():
                        if any(t == target_key_str and get_priority(c) >= min_positive_priority for t, c in deps):
                            source_info = next(
                                (info for info in path_to_key_info.values() if info.key_string == src_key_str), None
                            )
                            if source_info and source_info.norm_path not in all_excluded_abs:
                                if src_key_str not in relevant_keys_strings_set:
                                    logger.debug(
                                        f"  Adding suggested foreign key '{src_key_str}' linked to internal '{target_key_str}'"
                                    )
                                    relevant_keys_strings_set.add(src_key_str)

            logger.debug(
                f"Mini Tracker ({module_key_string}): Relevant keys after processing suggestions: {len(relevant_keys_strings_set)}"
            )
        ...
        logger.debug(
            f"Validating {len(relevant_keys_strings_set)} relevant keys against provided path_to_key_info map..."
        )
        ...
        if invalid_keys_found:
            logger.warning(
                f"Mini update ({os.path.basename(output_file)}): Excluding keys with no path found in provided map: {sorted(list(invalid_keys_found))}"
            )
        ...
        if keys_to_explicitly_remove:
            ...
            if keys_actually_removed_explicitly:
                logger.info(
                    f"Mini update ({os.path.basename(output_file)}): Explicitly removing keys as requested: {keys_actually_removed_explicitly}"
                )
        ...
        logger.info(
            f"Mini tracker update for module {module_key_string} ({os.path.basename(module_path)}). Grid keys: {len(relevant_keys_for_grid)}."
        )
        ...
        if raw_suggestions and file_to_module:
            logger.debug(
                f"Filtering mini-tracker suggestions against final valid keys ({os.path.basename(output_file)})..."
            )
            ...
            for src_key_str, deps in raw_suggestions.items():
                ...
                if not source_info or source_info.norm_path in all_excluded_abs:
                    continue
                # Add source to final_suggestions_to_apply only if it's in the final grid keys
                if src_key_str not in relevant_keys_for_grid:
                    continue
                ...
                for target_key_str, dep_char in deps:
                    ...
                    if not target_info or target_info.norm_path in all_excluded_abs:
                        continue
    ...
    if tracker_exists:
        try:  # Read logic unchanged
            ...
            logger.debug(
                f"Read existing tracker {os.path.basename(output_file)}: {len(existing_key_defs)} keys, {len(existing_grid)} grid rows."
            )
        except Exception as e:
            logger.error(f"Failed read existing tracker {output_file}: {e}. Proceeding cautiously.", exc_info=True)
            existing_key_defs = {}
            existing_grid = {}
            current_last_key_edit = ""
            current_last_grid_edit = ""
    ...
    if not tracker_exists:
        ...
        if tracker_type == "mini":
            created_ok = create_mini_tracker(
                module_path, path_to_key_info, sorted_keys_list_for_create, relevant_new_keys_str_list
            )
            if created_ok:
            ...
            last_key_edit_msg = (
                f"Assigned keys: {', '.join(relevant_new_keys_str_list)}"
                if relevant_new_keys_str_list
                else (
                    f"Initial keys: {len(sorted_keys_list_for_create)}"
                    if sorted_keys_list_for_create
            ...
            created_ok = write_tracker_file(
                output_file, final_key_defs, grid_after_creation, last_key_edit_msg, "Initial creation"
            )
    ...
    if relevant_new_keys_str_list:
        final_last_key_edit = f"Assigned keys: {', '.join(relevant_new_keys_str_list)}"  # Prioritize message about NEW keys added globally
    elif added_keys or removed_keys:
        change_parts = []
        if added_keys:
            change_parts.append(f"Added {len(added_keys)} keys")
        if removed_keys:
            change_parts.append(f"Removed {len(removed_keys)} keys")
    ...
    if tracker_type == "doc" or tracker_type == "mini":
        ...
        if key_string_to_info_local:  # Proceed only if map is not empty
            ...
            for row_key in final_sorted_keys_list:
                ...
                if not row_info or not row_info.is_directory:
                    continue  # Rules originate from directories
                ...
                for col_key in final_sorted_keys_list:
                    ...
                    if structural_deps.get(row_key, {}).get(col_key) is not None:
                        continue
    ...
    if use_old_map_for_migration:
        ...
        if old_path_to_key_info:
            ...
            if len(old_key_string_to_path) != len(old_path_to_key_info):
                logger.warning(
                    "Potential duplicate key strings detected in 'global_key_map_old.json'. Mapping might be ambiguous."
                )
        else:
            logger.warning(
                "'_old.json' map not found or failed to load, even though requested. Falling back to tracker's existing key definitions."
            )
    ...
    if old_key_string_to_path is None:  # Check if primary method failed or was skipped
        if existing_key_defs:
            ...
            logger.error(
                "Cannot copy old grid values: Neither usable '_old.json' map nor tracker's existing definitions are available."
            )
    ...
    skipped_due_to_no_old_path = 0
    skipped_due_to_path_gone = 0
    skipped_due_to_non_placeholder = 0
    ...
    if old_key_string_to_path is not None:
        ...
        for old_row_key, compressed_row in existing_grid.items():
            ...
            if new_row_idx is None:
                ...
                logger.debug(
                    f"  Skip row path '{old_row_path}' (old key '{old_row_key}'): Path no longer in final definitions."
                )
                continue
            ...
            try:
                ...
                if len(decomp_row) != len(old_keys_list):
                    logger.warning(
                        f"Grid Copy: Row length mismatch for '{old_row_key}'! Expected {len(old_keys_list)}, Got {len(decomp_row)}. Skipping row copy."
                    )
                    continue

                # Iterate through the OLD column indices
                for old_col_idx, value in enumerate(decomp_row):
                    if value in (DIAGONAL_CHAR, PLACEHOLDER_CHAR, EMPTY_CHAR):
                        continue
                    ...
                    if not old_col_path:
                        skipped_due_to_no_old_path += 1
                        logger.debug(
                            f"  Skip cell ({old_row_key}[{old_col_idx}]): Old col key '{old_col_key}' path not found."
                        )
                        continue
                    ...
                    if new_col_idx is None:
                        skipped_due_to_path_gone += 1
                        logger.debug(
                            f"  Skip cell ({old_row_key}[{old_col_idx}]): Col path '{old_col_path}' (old key '{old_col_key}') no longer in final definitions."
                        )
                        continue
                    ...
                    if new_row_idx != new_col_idx:  # Ensure not diagonal
                        ...
                        if target_cell_current_value == PLACEHOLDER_CHAR:
                            ...
                            logger.debug(
                                f"  Copied '{value}' from ({old_row_key},{old_col_key}) path ({old_row_path},{old_col_path}) to ({new_row_key},{new_col_idx})"
                            )
                        else:
                            skipped_due_to_non_placeholder += 1
                            logger.debug(
                                f"  Skip cell ({old_row_key}[{old_col_idx}]): Target ({new_row_key},{new_col_idx}) already has '{target_cell_current_value}'."
                            )

            except Exception as e:
                logger.warning(
                    f"Grid Rebuild: Error processing row '{old_row_key}' in {output_file}: {e}. Skipping copy.",
                    exc_info=False,
                )
        ...
        logger.warning(
            "Skipping grid value copy entirely as no authoritative old key-path mapping could be determined."
        )

    log_method = "None"
    if old_key_string_to_path is not None:
        log_method = "_old.json map" if using_old_map else "Tracker Fallback"
    logger.info(
        f"Finished copying old grid values. Method: {log_method}. Copied: {copied_values_count}, Skipped (No Old Path): {skipped_due_to_no_old_path}, Skipped (Path Gone): {skipped_due_to_path_gone}, Skipped (Target Filled): {skipped_due_to_non_placeholder}, RowErrors={row_processing_errors}"
    )
    ...
    if tracker_type == "mini" and module_path:  # Ensure we have module_path
        ...
        def is_path_in_doc_roots(item_path: str) -> bool:
            ...
            return any(
                is_subpath(norm_item_path, doc_root) or norm_item_path == doc_root for doc_root in abs_doc_roots_set
            )
        ...
        for i in range(len(foreign_key_list)):
            ...
            for j in range(i + 1, len(foreign_key_list)):
                        ...
                        if home_fk1_idx is not None and home_fk2_idx is not None:
                            ...
                            if row1_comp:
                                try:
                                    ...
                                    logger.warning(f"  Error decompressing home row {fk1_str}: {e}")
                            ...
                            if row2_comp:
                                try:
                                    ...
                                    logger.warning(f"  Error decompressing home row {fk2_str}: {e}")
                ...
                if changed_12 or changed_21:
                    logger.info(
                        f"  Updated relationship for ({fk1_str}, {fk2_str}) based on home tracker / auto-n rule. New: ('{final_char_12}', '{final_char_21}')"
                    )
    ...
    if final_suggestions_to_apply:
        logger.debug(
            f"Applying {sum(len(v) for v in final_suggestions_to_apply.values())} suggestions to grid for {output_file} (Force Apply: {force_apply_suggestions})"
        )
            ...
            for target_key, dep_char in deps:
                ...
                if force_apply_suggestions:
                    ...
                    if dep_char != PLACEHOLDER_CHAR and existing_char_in_grid != dep_char:
                        ...
                        logger.debug(
                            f"Force apply triggered for {row_key}->{col_key} ('{dep_char}') over ('{existing_char_in_grid}')"
                        )
                    ...
                    and existing_char_in_grid != dep_char
                ):
                    ...
                    try:
                        ...
                        if existing_char_in_grid != "n" and suggestion_priority > existing_priority:
                            logger.info(
                                f"Suggestion Conflict RESOLVED in {os.path.basename(output_file)}: Applying '{dep_char}' (prio {suggestion_priority}) over '{existing_char_in_grid}' (prio {existing_priority}) for {source_key}->{target_key}."
                            )
                            should_apply_suggestion = True  # Set flag to apply suggestion
                        else:
                            logger.debug(
                                f"Suggestion Conflict Ignored: Grid '{existing_char_in_grid}' kept over sugg '{dep_char}' for {source_key}->{target_key}."
                            )
                    except Exception as e:
                        logger.error(f"Priority check error: {e}. Grid value kept.")
                ...
                if should_apply_suggestion:
                    ...
                    if dep_char in ("<", ">"):
                        ...
                        if reverse_decomp_row and row_idx < len(reverse_decomp_row):
                            ...
                            if char_in_reverse == dep_char:
                                # <<< RESTORE MUTUAL LOGS >>>
                                logger.debug(
                                    f"Mutual: Merging {target_key} -> {source_key} to 'x' due to matching '{dep_char}' dependencies."
                                )
                                logger.debug(
                                    f"Mutual: Merging {source_key} -> {target_key} to 'x' due to matching '{dep_char}' dependencies."
                                )
                                # <<< END RESTORE >>>
                    ...
                    if existing_char_in_grid != final_char_to_apply:  # Check again in case 'x' upgrade happened
                        logger.debug(
                            f"Applying to grid: {source_key} -> {target_key} = '{final_char_to_apply}' (Force: {force_apply_suggestions}, Sugg: '{dep_char}', Existed: '{existing_char_in_grid}')"
                        )
                        current_decomp_row[col_idx] = final_char_to_apply
                        suggestion_applied = True
                        applied_this_iter = True
                        logger.debug(
                            f"Applied suggestion: {source_key} -> {target_key} ({final_char_to_apply}) in {output_file}"
                        )
                        ...
                        if reciprocal_char:
                            ...
                            if reverse_decomp_row and row_idx < len(reverse_decomp_row):  # Use source's index (row_idx)
                                ...
                                if force_apply_suggestions:
                                    ...
                                    if char_in_reverse == PLACEHOLDER_CHAR or get_priority(
                                        reciprocal_char
                                    ) > get_priority(char_in_reverse):
                                        should_apply_reciprocal = True
                                if should_apply_reciprocal:
                                    logger.debug(
                                        f"Reciprocal Apply: Setting {target_key} -> {source_key}  ('{reciprocal_char}') (Force: {force_apply_suggestions}, Existed: '{char_in_reverse}')"
                                    )
                ...
                # --- Handle Conflicts if suggestion NOT applied ---
                    ...
                    try:
                        ...
                        if existing_char_in_grid != "n" and suggestion_priority > existing_priority:
                            ...
                            logger.info(
                                f"Suggestion Conflict RESOLVED in {os.path.basename(output_file)}: For {source_key}->{target_key}, "
                        ...
                        logger.error(
                            f"Character priority lookup failed for '{str(e)}' during suggestion conflict check. Grid value kept."
                        )
                    except Exception as e:
                ...
                if force_apply_suggestions and applied_this_iter:
                    ...
                    if applied_manual_source is None:
                        applied_manual_source = source_key
                    elif applied_manual_source != source_key:
                        logger.warning("Multiple source keys detected during forced apply; message may simplify.")
                    ...
                    if applied_manual_dep_type is None:
                        applied_manual_dep_type = current_applied_type
                    elif applied_manual_dep_type != current_applied_type:
                        logger.warning("Multiple dependency types detected during forced apply; message may simplify.")
    ...
    if (
        ...
        and suggestion_applied
        and applied_manual_source
        and applied_manual_targets
        and applied_manual_dep_type
    ):
        # Use the captured details for the specific message
        sorted_targets = sort_key_strings_hierarchically(
            list(set(applied_manual_targets))
        )  # Use set to remove duplicates before sort
    ...
    try:
        ...
        if is_mini and lines:
            ...
            try:
                ...
                if mini_tracker_start_index >= mini_tracker_end_index:
                    raise ValueError("Start marker after end marker.")
            except (StopIteration, ValueError) as e:
                logger.warning(f"Mini markers invalid in {output_file}: {e}. Overwriting.")
                mini_tracker_start_index = -1
        ...
        with open(output_file, "w", encoding="utf-8", newline="\n") as f:
            ...
            _write_key_definitions(
                f, final_key_defs, final_sorted_keys_list
            )  # Write DEFINITIONS using final set of keys
            f.write("\n")
                                ...
                                for j, dep_type in enumerate(decompressed_row):
                                    if dep_type not in (EMPTY_CHAR, DIAGONAL_CHAR, PLACEHOLDER_CHAR):
                                        ...
                                        writer.writerow(
                                            [
                                                source_key,
                                                keys_map.get(source_key, ""),
                                                target_key,
                                                keys_map.get(target_key, ""),
                                                dep_type,
                                            ]
                                        )
                            else:
                                logger.warning(f"CSV Export: Row length mismatch for key '{source_key}'")
                        except Exception as e:
                            logger.warning(f"CSV Export: Error processing row for '{source_key}': {e}")
                                ...
                                for j, dep_type in enumerate(decompressed_row):
                                    if dep_type not in (EMPTY_CHAR, DIAGONAL_CHAR, PLACEHOLDER_CHAR):
                                        ...
                                        if dep_type == ">":
                                            ...
                                            style = "bold"
                                        f.write(
                                            f'  "{source_key}" -> "{target_key}" [label="{dep_type}", color="{color}", style="{style}", arrowhead="{arrowhead}"];\n'
                                        )
                            else:
                                logger.warning(f"DOT Export: Row length mismatch for key '{source_key}'")
                        except Exception as e:
                            logger.warning(f"DOT Export: Error processing row for '{source_key}': {e}")
                f.write("}\n")
        else:
...
def remove_key_from_tracker(output_file: str, key_to_remove: str):
    ...
    if key_to_remove not in local_keys:
        logger.warning(
            f"Key '{key_to_remove}' not found in the definitions of tracker '{output_file}'. No update needed."
        )
    ...
    if path_removed in modified_path_to_key_info:
        ...
        logger.warning(
            f"Path '{path_removed}' (from key '{key_to_remove}' in {output_file}) not found as a key in the loaded global map. Proceeding, but this might indicate an inconsistency."
        )
